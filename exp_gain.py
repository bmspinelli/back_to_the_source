...
def exp_gain(d, paths_pred, time, s_1, observations, candidates_set, s, approx, 
        noise=0, tol_c=1):

    """Compute the expected gain"""
    
    pos_obs, neg_obs = observations[0], observations[1]
    first_observation = (s_1, pos_obs[s_1])
...
def h_compatible(d, first_observation, cand, c, t_c, noise=0, tol_c=1):
    """ Select candidates sources that are compatible with c being infected at
    time t_c
    
    """
    (o_1, t_1) = first_observation
    compatibility = (abs(d[o_1][cand] - d[c][cand] - t_1 + t_c) <=
                ut.tol(d[o_1][cand]+d[c][cand], noise=noise, tol_c=tol_c))
...
def later_compatible(d, time, first_observation, cand, c, noise=0,
        tol_c=1):
    ...
    if time == np.inf:
        ...
        compatibility = (d[o_1][cand] + time - t_1 < d[c][cand] +
                ut.tol(d[o_1][cand]+d[c][cand], noise=noise, tol_c=tol_c))
        return int(compatibility)
